...
from aft.tools.threadhandler import ThreadHandler as thread_handler
    ...
    try:
        ...
        if not args.notest:
            ...
            if not args.nopoweroff:
                device.detach()
                if args.emulateusb:
                    device_manager.stop_image_usb_emulation(device.leases_file_name)
...
def parse_args():
    ...
    parser.add_argument(
        "--testplan", type=str, nargs="?", action="store", default="",
